...
public class ObjectPrimary {
     ...
     * 属性列表
     */
    private final Map<String, ObjectProperty> properties = Collections.mapOf();
    ...
    public ObjectPrimary(Class<?> primary) {
        ...
        /* init */
        Map<String, ObjectProperty> map = getPrimaryProperties(primary, Collections.listOf()).stream()
    ...
    public List<ObjectProperty> getProperties(boolean isFilter) {
        if (!isFilter)
            return Collections.listOf(properties.values());
        /* 筛选出修饰符非 static & final 的属性列表 */
        return Collections.listFilter(properties.values(), value -> (!value.isStatic() && !value.isFinal()));
    }
