package org.forironflower.vegalib.test.io;
...
public class ByteBufTest {
    ...
    public void readLong() {
        ...
        buffer.write(1024L);
        buffer.seek(ByteBuf.SEEK_SET, 0);
        System.out.println(buffer.readLong());

        buffer.write(2048L);
        buffer.seek(ByteBuf.SEEK_SET, 8);
        System.out.println(buffer.readLong());
    }
    ...
    public void readTest() {
        ...
        buf.seek(ByteBuf.SEEK_SET, 0);
        while ((len = buf.read(nbuf, 0, nbuf.length)) != -1)
            IOUtils.stdout.println("read len=%s, nbuf=%s", len, Arrays.toString(nbuf));
